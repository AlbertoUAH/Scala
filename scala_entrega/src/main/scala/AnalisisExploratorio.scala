...
object AnalisisExploratorio extends Analizador {
  ...
  def paisesOrigenUnicos(c: Seq[Contribuyente]): Seq[String] = {
    c.map(contrib => if (contrib.nativeCountry.trim.isEmpty) "NA" else contrib.nativeCountry).toSet.toSeq
  }
  ...
  def trabajoMejorRemunerado(c: Seq[Contribuyente]): String = {
    ...
    val trabajosMejorRemunerados = porcentajeMejorRemunerado.find(_._2>50).toSeq
    trabajosMejorRemunerados.sortBy(- _._2).toSeq(0)(0).toString()
  }
  ...
  //  println(s" -> Los contribuyentes provienen de distintos países como: ${paisesOrigenUnicos(c = dataset).mkString(" , ")}")
